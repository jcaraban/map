...
string defines_focal_type(DataType data_type) {
	...
	string ctype = data_type.ctypeString();
	string str =
	"""" "#define LOAD_F_"+type+"(x) load_F_"+type+"(VAR_LIST(x),bc0,bc1,BS0,BS1,BC0,BC1,BN0,BN1)"
	...
	"\n" +ctype+" load_F_"+type+"(TYPE_VAR_LIST("+ctype+",IN), int bc0, int bc1, int BS0, int BS1, int BC0, int BC1, int BN0, int BN1) {"
	"\n" "	global const "+ctype+" *p = IN;"
	"\n" "	"+ctype+" v = INv;"
	"\n" "	uchar f = INf;"
	"\n" "	int i0 = (bc0<0) ? 0 : (bc0>=BS0) ? 2 : 1;"
	"\n" "	int i1 = (bc1<0) ? 0 : (bc1>=BS1) ? 2 : 1;"
	"\n" "	// normal case, fixed and pointer"
	"\n" "	int center = (i0 == 1 && i1 == 1);"
	"\n" "	if (center)"
	"\n" "		return (f) ?  v : p[(bc1)*BS0+bc0];"
	"\n" "	// border cases"
	"\n" "	int north = (i1 == 0 && i0 == 1);"
	"\n" "	if (north) { p = IN_01; v = INv_01; f = INf_01; }"
	"\n" "	int west = (i1 == 1 && i0 == 0);"
	"\n" "	if (west) { p = IN_10; v = INv_10; f = INf_10; }"
	"\n" "	int east = (i1 == 1 && i0 == 2);"
	"\n" "	if (east) { p = IN_12; v = INv_12; f = INf_12; }"
	"\n" "	int south = (i1 == 2 && i0 == 1);"
	"\n" "	if (south) { p = IN_21; v = INv_21; f = INf_21; }"
	"\n" "	int north_west = (i1 == 0 && i0 == 0);"
	"\n" "	if (north_west) { p = IN_00; v = INv_00; f = INf_00; }"
	"\n" "	int north_east = (i1 == 0 && i0 == 2);"
	"\n" "	if (north_east) { p = IN_02; v = INv_02; f = INf_02; };"
	"\n" "	int south_west = (i1 == 2 && i0 == 0);"
	"\n" "	if (south_west) { p = IN_20; v = INv_20; f = INf_20; }"
	"\n" "	int south_east = (i1 == 2 && i0 == 2);"
	"\n" "	if (south_east) { p = IN_22; v = INv_22; f = INf_22; };"
	"\n" "	// border fixed"
	"\n" "	if (f)"
	"\n" "		return v;"
	"\n" "	// border pointer"
	...
	"\n" "	if (i1 == 0 && i0 == 0) {"
	"\n" "		if (BC0 != 0) {"
	"\n" "			p = IN_10; v = INv_10; f = INf_10;"
	"\n" "			bc0 = modulo(bc0,BS0);"
	"\n" "			bc1 = invert(bc1,BS1);"
	"\n" "		} else if (BC1 != 0) {"
	"\n" "			p = IN_01; v = INv_01; f = INf_01;"
	"\n" "			bc0 = invert(bc0,BS0);"
	"\n" "			bc1 = modulo(bc1,BS1);"
	"\n" "		} else { // BCO,1 == {0,0}"
	"\n" "			p = IN; v = INv; f = INf;"
