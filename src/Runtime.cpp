...
#include "Runtime.hpp"
//#include "intermediate/dag/util.hpp"
...
void Runtime::setupDevices(std::string plat_name, DeviceType dev, std::string dev_name) {
	...
	// Adding 1 context
	const int dev_num = 1;
	cl_device_id dev_group[dev_num] = {*clenv.D(0)};
	cl_context_properties cp[3] = {CL_CONTEXT_PLATFORM, (cl_context_properties)*clenv.D(0).P(), 0};
	cl_int err;
	cl_context ctx = clCreateContext(cp, dev_num, dev_group, NULL, NULL, &err);
	cle::clCheckError(err);
...
void Runtime::evaluate(NodeList list_to_eval) {
	...
	// Attempts to setup CPU device
	if (clenv.contextSize() == 0) {
...
//print_nodes(priv_list); // @ Prints nodes
	...
	// closes open files // @@
	for (auto node : graph)
		if (auto write = dynamic_cast<Write*>(node))
			write->getFile()->close();
...
void Runtime::workflow(NodeList list) {
	...
	program.compose(fusioner.sorted_cluster_list);
