 ...
 * TODO: pinned_list now has 1 cl_mem per worker, it would need 'max_io_block' if events are activated
 * TODO: create a pool of Blocks (like 'pinned_list') so that blocks are reused instead than re-allocated non-stop?
 *
 * TODO: integrate 'file_hash' with 'file_count' and make the shared_ptr destructor call 'releaseFile()' automatically
 */
...
#include "../file/BinaryFile.hpp" // @ needed for retainFile
#include <algorithm>
...
void Cache::allocChunks(cle::Context ctx) {
	...
	block_page = clCreateBuffer(*ctx, CL_MEM_READ_WRITE, conf.cache_block_size, nullptr, &err);
	cle::clCheckError(err);
...
void Cache::freeChunks() {
	...
	err = clReleaseMemObject(block_page);
	cle::clCheckError(err);
...
void Cache::allocEntries(const Program &prog) {
	...
	// @
	block_order.resize(conf.num_ranks,0);
}
...
void Cache::requestBlocks(const KeyList &key_list, BlockList &blk_list) {
	...
	for (auto &tuple : key_list) {
		...
		{
			blk_list.push_back( new Block1(key,dep,block_page) );
		...
		blk_list.back()->order = block_order[idx]++; // @
	}
}
...
Block* Cache::retainBlock(const Key &key, int depend) {
	if (not conf.inmem_cache) { // no-cache mode
		Block *blk = new BlockN(key,DEPEND_UNKNOWN,unit_mem_size);
		blk->setFile(retainFile(blk->key));
		return blk; // always creates / deletes the block
	}
	return retainBlock_(key,depend); // cache mode
}

Block* Cache::retainBlock_(const Key &key, int depend) {
...
std::shared_ptr<IFile> Cache::retainFile(Key key) {
	...
	if (it == file_hash.end())
	{
		...
		// @@ not working with Radial
		// Updates 'count' for 'no_cache' mode
		if (not conf.inmem_cache) {
			for (auto task : Runtime::getProgram().taskList()) {
				if (is_included(key.node,task->inputList()))
				{
					auto num_block = task->numblock();
					auto beg = Coord(num_block.size(),0);
					auto end = num_block;
					for (auto coord : iterSpace(beg,end))
					{
						auto reach = task->accuInputReach(key.node,coord);
						auto space = reach.blockSpace(task->blocksize());
						for (auto offset : space)
							count += all(in_range(coord+offset,num_block));
					}
				}
			}
		...
		//std::cout << "retain_file " << key.node->id << ": " << count << std::endl;
	}
...
void Cache::retainEntry(Block *blk) {
	if (not conf.inmem_cache) { // no-cache mode
		int idx = Tid.rnk()*conf.max_io_block + blk->order;
		blk->setEntry( &entry_list[idx] );
		assert(not blk->isUsed());
		blk->setUsed();
		return;
	}
	retainEntry_(blk); // cache mode
}

void Cache::retainEntry_(Block *blk) {
	std::unique_lock<std::mutex> lock(blk->mtx); // thread-safe
...
void Cache::releaseEntry_(Block *blk) {
	std::unique_lock<std::mutex> lock(blk->mtx); // thread-safe
...
void Cache::releaseBlock(const Key &key, Block *blk) {
	...
	releaseBlock_(key,blk); // cache mode
}

void Cache::releaseBlock_(const Key &key, Block *blk) {
	std::unique_lock<std::mutex> lock(mtx_blk); // thread-safe
	bool erase = false;
...
void Cache::releaseFile(Key key) {
	...
	if (it->second == 0) { // Releases the 'files' when done
		file_hash.erase(key);
//std::cout << "release_file " << key.node->id << std::endl;
	}
}
