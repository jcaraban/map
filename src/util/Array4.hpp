...
inline Coord next(Coord coord, Coord start, Coord stop, int step=1) {
	assert(coord.size() == start.size());
	assert(start.size() == stop.size());
	assert(not coord.isNone());
	assert(all(start <= stop));
	assert(any(coord < stop));
	int carry = 0;
...
inline std::vector<Coord> iterSpace(Coord start, Coord stop) {
	assert(start.size() == stop.size());
	if (start.size() == 0)
	...
	while (all(coord < stop)) {
		vec.push_back(coord);
		coord = next(coord,start,stop);
	}
	return vec;
}
