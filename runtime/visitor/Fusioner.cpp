...
//print(); // @
	...
	print(); // Prints clusters and nodes once linked & sorted
}
...
void Fusioner::forwarding(std::function<bool(Node*)> for_pred) {
	...
	auto all_nodes = [](Cluster *cluster) { // @ change to full_unique_join ?
		auto body_out = full_join(cluster->nodeList(),cluster->outputList());
		std::unique(body_out.begin(),body_out.end());
		return full_join(cluster->inputList(),body_out);
	};
...
void Fusioner::sorting() {
	...
	// Topological sort of cluster_list, in order of dependencies and first-node id
	auto less = [](const std::unique_ptr<Cluster> &a, const std::unique_ptr<Cluster> &b){
		Cluster *ga = a.get(), *gb = b.get();
	...
	std::sort(cluster_list.begin(),cluster_list.end(),less);
	
	// Numerate clusters with an 'id' after sorting
	for (auto &cluster : cluster_list) {
		cluster->id = Cluster::id_count++;
	}
